...
def process(buyers, source_path, file_prefix, output_folder, chosen_size=None):
    ...
    PERMS = (
        UserAccessPermissions.PRINT |                       # printing allowed
        UserAccessPermissions.PRINT_TO_REPRESENTATION |     # high-resolution printing
        UserAccessPermissions.EXTRACT |                     # copy text, images and other content
        UserAccessPermissions.EXTRACT_TEXT_AND_GRAPHICS |   # extract for accessibility (kept from before)
        UserAccessPermissions.ADD_OR_MODIFY |               # commenting / text annotations
        UserAccessPermissions.FILL_FORM_FIELDS              # fill in form fields + sign existing signature fields
    )
    ...
    for i, (name, owner_password, view_password) in enumerate(buyers, 1):
        ...
        writer = PdfWriter()
        writer.append(reader)   # writer owns clean copies of every page; merge onto these
